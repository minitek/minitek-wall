...
if (!empty($this->wall) ||  $this->wall !== 0) {
	...
	foreach ($this->wall as $key => $item) {
		...
		$item_classes = $catfilter . ' '
			...
			. $datefilter . ' '
			. $this->hoverOffset;
		$item_style = 'padding: ' . (int)$this->gutter . 'px; ' . $this->cols;
		...
		if ($this->mas_border_radius) {
			$inner_cont_style .= 'border-radius: ' . $this->mas_border_radius . 'px; ';
		}
		if ($this->mas_border) {
			$inner_cont_style .= 'border: ' . $this->mas_border . 'px solid ' . $this->mas_border_color . ';';
		}

		$img_style = '';
		if ($options['position_class'] != 'content-below') {
			if ($this->mas_border_radius) {
				$img_style .= 'border-radius: ' . $this->mas_border_radius . 'px; ';
			}

			if ($this->mas_border) {
				$img_style .= 'border: ' . $this->mas_border . 'px solid ' . $this->mas_border_color . ';';
			}
		} ?>

		<div class="mwall-item <?php echo $item_classes; ?>" style="<?php echo $item_style; ?>" <?php echo $item_data_attributes; ?>>
			<div class="mwall-item-outer-cont <?php echo $position_class; ?>" style="<?php echo $this->animated_flip; ?>">
				<div class="mwall-item-inner-cont" style="<?php echo $inner_cont_style; ?>">
					<?php if (!$empty_media) { ?>
						<div class="mwall-cover <?php echo $this->hoverOffset; ?> <?php echo $this->perspective; ?>">
							<div class="mwall-img-div <?php echo $this->flipBase; ?> <?php echo $this->flipClass; ?>" style="<?php echo $this->animated_flip; ?> <?php echo $img_style; ?>">
								...
								<?php
								$inner_classes = $options['title_class'] . ' '
								...
								if ($empty_media) {
									$inner_classes .= ' mwall-no-image';
								}

								$inner_style = 'background-color: rgba(' . $options['db_bg_class'] . ',' . $options['db_bg_opacity_class'] . ');';

								if ($options['position_class'] != 'content-below' && $this->detailBoxAll) { ?>
									<div class="mwall-item-inner mwall-detail-box <?php echo $inner_classes; ?>" style="<?php echo $inner_style; ?>">
					...
					$item_inner_style = 'background-color: rgba(' . $options['db_bg_class'] . ',' . $options['db_bg_opacity_class'] . ');';

					if ($empty_media || ($this->detailBoxAll && ($options['position_class'] == 'content-below'))) { ?>
						<div class="mwall-item-inner mwall-detail-box <?php echo $item_inner_class; ?>" style="<?php echo $item_inner_style; ?>">
							<?php if ($this->detailBoxDateAll && isset($item->itemDate)) { ?>
								<div class="mwall-date">
									<?php echo $item->itemDate; ?></div>
							<?php }
							...
							<div class="mwall-hover-box <?php echo $this->hoverClass; ?>" style="<?php echo $hover_style; ?>">
								<div class="mwall-hover-box-content">
									...
									if ($this->hoverBoxIntrotext) {
										if (!empty($item->hover_itemIntrotext)) { ?>
											<div class="mwall-desc">
												<?php echo $item->hover_itemIntrotext; ?></div>
										<?php
										}
									}
