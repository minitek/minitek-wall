				...
				<div class="mwall-item-inner-cont">
					<?php if (!empty($item->itemImage)) {
						if (isset($item->itemLink) && $this->mas_image_link) { ?>
							<a href="<?php echo $item->itemLink; ?>" class="mwall-photo-link" style="background-image: url('<?php echo $item->itemImage; ?>');"></a>
						<?php } else { ?>
							<div class="mwall-photo-link" style="background-image: url('<?php echo $item->itemImage; ?>');"></div>
						<?php }
					}

					if ($this->detailBoxAll) { ?>
						<div class="mwall-item-inner mwall-detail-box <?php echo $empty_media ? 'mwall-no-image' : ''; ?>">
